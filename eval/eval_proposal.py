...
def average_recall_vs_avg_nr_proposals(ground_truth, proposals,
    ...
    for ridx, tiou in enumerate(tiou_thresholds):
        ...
        recall_values = matches.sum(axis=0) / positives.sum()
        area_under_curve = np.trapz(recall_values, pcn_lst)

        plt.plot(pcn_lst*100, recall_values, linestyle='--', label='tiou={}, area={:.2f}'.format(tiou, area_under_curve))
    ...
    area_under_curve = np.trapz(avg_recall, proposals_per_video)
    plt.plot(proposals_per_video, avg_recall, label='tiou=0.1,0.7,7, area={:.2f}'.format(area_under_curve))
    plt.legend()
    plt.savefig("image_eval/AR-AN.png")
    plt.close()
    #plt.show()
    return recall, avg_recall, proposals_per_video
